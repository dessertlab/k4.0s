...
import (
	...
	"k8s.io/kubernetes/pkg/scheduler/framework/plugins/nodeassurancescorer"
	...
	"k8s.io/kubernetes/pkg/scheduler/framework/plugins/rtschedulability"
	"k8s.io/kubernetes/pkg/scheduler/framework/plugins/selectorspread"
	"k8s.io/kubernetes/pkg/scheduler/framework/plugins/serviceaffinity"
	"k8s.io/kubernetes/pkg/scheduler/framework/plugins/tainttoleration"
	"k8s.io/kubernetes/pkg/scheduler/framework/plugins/tdmascheduling"
...
const (
	...
	NodeAssuranceScorer = "NodeAssuranceScorer"

	Rtschedulability = "Rtschedulability"

	TDMAscheduling = "TDMAscheduling"
)
...
type ConfigProducerArgs struct {
	...
	NodeAssuranceScorerArgs *config.NodeAssuranceScorerArgs
	TDMAschedulingArgs      *config.TDMAschedulingArgs
}
...
func NewLegacyRegistry() *LegacyRegistry {
	...
	registry.registerPredicateConfigProducer(Myplugin,
		func(_ ConfigProducerArgs, plugins *config.Plugins, _ *[]config.PluginConfig) {
			plugins.Filter = appendToPluginSet(plugins.Filter, myplugin.Name, nil)
		})
	registry.registerPredicateConfigProducer(Rtschedulability,
		func(args ConfigProducerArgs, plugins *config.Plugins, _ *[]config.PluginConfig) {
			plugins.Filter = appendToPluginSet(plugins.Filter, rtschedulability.Name, nil)
			plugins.Score = appendToPluginSet(plugins.Score, rtschedulability.Name, &args.Weight)
			plugins.PreBind = appendToPluginSet(plugins.PreBind, rtschedulability.Name, nil)
		})
	registry.registerPredicateConfigProducer(TDMAscheduling,
		func(args ConfigProducerArgs, plugins *config.Plugins, pluginConfig *[]config.PluginConfig) {
			plugins.PreFilter = appendToPluginSet(plugins.Filter, tdmascheduling.Name, nil)
			plugins.Reserve = appendToPluginSet(plugins.PreBind, tdmascheduling.Name, nil)
			if args.TDMAschedulingArgs != nil {
				*pluginConfig = append(*pluginConfig,
					config.PluginConfig{Name: tdmascheduling.Name, Args: args.TDMAschedulingArgs})
			}
		})
	...
	registry.registerPriorityConfigProducer(NodeAssuranceScorer,
		func(args ConfigProducerArgs, plugins *config.Plugins, pluginConfig *[]config.PluginConfig) {
			//plugins.PreScore = appendToPluginSet(plugins.PreScore, tainttoleration.Name, nil)
			plugins.Score = appendToPluginSet(plugins.Score, nodeassurancescorer.AsssuranceScorerName, &args.Weight)
			if args.NodeAssuranceScorerArgs != nil {
				*pluginConfig = append(*pluginConfig,
					config.PluginConfig{Name: nodeassurancescorer.AsssuranceScorerName, Args: args.NodeAssuranceScorerArgs})
			}
		})
